...
```bash
bunx react-doctor --project apps/riptide-ui --diff false --yes
```
...
A pi agent runs with the repo's `react-doctor` skill. Its per-issue loop gives the agent three honest options — **fix**, **ignore** (add to `doctor.config.ts` with a reason), or **skip** (leave for a human) — and validates each change before committing it separately:

```bash
bun run typecheck
bun run quality
bunx react-doctor --project apps/riptide-ui --staged   # no new issues in staged files
```
...
## The loop — `pi-react-doctor.yml`

A scheduled workflow (daily, plus manual dispatch and `/iterate`) runs the loop through pi and opens a PR whose body is the agent's final message. Because the sensor, controller, and actuator are fused into one agent step here, the workflow does **not** have three separate steps — it collapses them, matching the design. A loop with a deterministic, standalone sensor and controller would instead have discrete steps.
...
- Maintainers comment `/iterate` on the PR; a hidden marker in the PR body routes the comment to the workflow that created it, which loads the PR context and feedback and updates the memory file and the PR. Deterministic glue lives in `ci-scripts/` (`agent-iteration.ts` and the workflow's pi output capture).

## Flow control

Every PR is labeled `pi-agent-react-doctor`. Scheduled runs no-op when an open PR with that label already exists, so there is at most one open PR per loop; manual dispatch bypasses the gate.
...
- **Not reusable:** `react-doctor`, the pi command, the "top 3 rules / 5 fixes" policy, the `riptide-ui` scope. Those are tailored to this repo — yours come from your interview.
