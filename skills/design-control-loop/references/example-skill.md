...
For an example recurring GitHub Actions workflow that runs this skill through pi, generate one from `references/workflow-template.yml`; place its standing feedback in the target repository's documented agent-memory path.
...
### 11. Read the response template and format your final answer
Read the response template at `references/response-template.md` and format your answer accordingly.
