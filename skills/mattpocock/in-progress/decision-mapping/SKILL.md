...
- **Prototype**: Writing UI or logic code to test a hypothesis, or to explore a design space. Uses the `/skill:prototype` skill. Creates a prototype as an asset. Use this when "how should it look" or "how should it behave" is the key question.
- **Grilling**: Conversation with the agent. Uses the `/skill:grill-with-docs` and `/skill:domain-modeling` skills. Asks one question at a time. The default case.
...
1. Run a `/skill:grill-with-docs` + `/skill:domain-modeling` session to surface the open decisions. Ask one question at a time.
...
2. Run a session to resolve the ticket, invoking skills as needed. If in doubt, use `/skill:grill-with-docs` and `/skill:domain-modeling`.
