...
Spawn 3+ subagents in parallel using the `subagent` tool with `agent: worker` and mode `parallel` (one `tasks` array, one entry per design). Each must produce a **radically different** interface for the deepened module.
